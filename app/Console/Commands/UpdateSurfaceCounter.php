...
//        Log::info("Update completed! Updated: $updatedCount | Disabled: $disabledCount");
        ...
        $folderPath = storage_path('app/public/tiles'); // Path to storage folder
        ...
        foreach ($files as $file) {
            if ($file->getSize() > $sizeLimit) {
                $largeImages[] = $file->getFilename(); // ✅ Store only file names
            }
        }

        if (!empty($largeImages)) {
            $fileNamesString = implode(',', $largeImages); // ✅ Convert to comma-separated string
            Log::info("Large Images Found: " . $fileNamesString); // ✅ Log to Laravel log file
        } else {
            Log::info("No large images found.");
        }

        //return response()->json(['large_images' => $largeImages]); // ✅ Return JSON properly
