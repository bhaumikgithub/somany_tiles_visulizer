...
class Savedroom extends Model
{
    public function getImageAttribute($value)
    {
        ...
        return $this->room->iconfile;
    }

    public function getUrlWithParamsAttribute()
    {
        if (config('app.url_filters')) {
            return $this->url . $this->note;
        }
    ...
    public function del()
    {
        ...
        if ($this->engine == 'panorama' && $this->note == 'backed') {
            Storage::disk('public')->deleteDirectory('savedrooms/' . $this->url);
        }
    ...
    public static function getUserSavedRooms1($userId,$skip,$rowperpage)
    {
      
        if (isset($userId)) {
            return Savedroom::where([['userid', $userId], ['enabled', 1]])
                ->whereIn('engine', Savedroom::getEngines())
                ->orderBy('updated_at', 'desc')->skip($skip)->take($rowperpage)->get();
                // ->paginate(20);
