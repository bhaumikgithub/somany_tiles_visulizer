...
{
    if ($('#open_ai_modal').hasClass('in')) {
        $('#open_ai_modal').modal('hide');
    }
}

// function cmdAppliedTiles()
// {   
//     // Open drawer if not already open
//     let drawer = $('#topPanel');
//     let isDrawerOpen = drawer.is(':visible') && drawer.css('right') === '0px';
//     if (!isDrawerOpen) {
//         drawer.show().animate({ right: '0px' });
//         console.log('Drawer opened');
//     } else {
//         console.log('Drawer already open');
//         // 2. Switch to Wall A
//         if ($('#slected-panel').css('display') === 'block') {
//             surfaceName = $('#slected-panel .display_surface_name h5#optionText').html();
//             if (surfaceName) {
//                 if (surfaceName === 'Wall A') {
//                     $('#slected-panel').hide();
//                     $('#selectd-data').show();
//                     $('#list_wall_A img').attr('src', 'http://127.0.0.1:8000/storage/tiles/6811a2054dc77.jpg');
//                     hideAIModal();
//                 } else {
//                     console.log(`Currently in ${surfaceName}, switching to Wall A...`);
//                     openTileSelectionPanel('Wall_A');
//                     $('#selectd-data').show();
//                     $('#list_wall_A img').attr('src', 'http://127.0.0.1:8000/storage/tiles/6811a2054dc77.jpg');
//                     hideAIModal();
//                 }
...
//             }
//         } else {
//             console.log('slected-panel is hidden. No active surface.');
//         }
//     }
...
{   
    if ($('#slected-panel').css('display') === 'block') {
        let surfaceName = $('#slected-panel .display_surface_name h5#optionText').text();
        let tileName = "ASPIRE GRANDE EL VOLCANO GREY FP";

        if (!tileName || !surfaceName) {
            alert("Tile name and surface name are required.");
            return;
        }
        if ( surfaceName != "Wall A" ) {
            openTileSelectionPanel('Wall_A');
        ...
        // Step 2: Find tile in list
        let tileElement = $('#topPanelTilesListUl li.top-panel-content-tiles-list-item').filter(function () {
        ...
        const tileElementId = tileElement.attr('id'); // e.g., tile_0
        const tileId = tileElement.data('tile'); // e.g., 499
        const tileImage ="http://127.0.0.1:8000/storage/tiles/6811a2054dc77.jpg";
        const caption = tileElement.find('p.-caption').text().trim();
        const finish = tileElement.data('finish'); // e.g., 499
        ...
        const p_tile = {
            ...
            icon: tileImage,
            text: caption,
            theme_thumbnail: tileImage // fallback if using themes
        };

        let clickedSurface = findRoomSurfaceUsingName(surfaceName);
        ...
        clickedTiles(p_tile , surfaceName);
        getTileId(tileElementId);
    }
