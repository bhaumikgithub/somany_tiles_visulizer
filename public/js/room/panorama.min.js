...
var currentRoom;
var roomTextures;
var currentRoomMaterial;

var topPanelTilesListUl = document.createElement('ul');
topPanelTilesListUl.id = 'topPanelTilesListUl';
topPanelTilesListUl.classList.add('panel_list'); // Adding the class

var guiObj = {
    hideTiles: { value: false },
    animate: { value: true },
    rotateWithRoom: { value: true },
    shininess: { value: 5 },
    specularPower: { value: 3.40 },
    specColPower: { value: 1.3 },
    enableLights: { value: [true, true, true] },
    lightOptions: [
        { distance: 30, speed: 0.0049, angle: 0 },
        { distance: 30, speed: 0.0049, angle: 2.1 },
        { distance: 30, speed: 0.0049, angle: 4.2 },
    ],
    positions: {
        value: [
...
if (window.orientation !== undefined) {
    ...
    var el = document.getElementById('bottomMenuFullScreen');
    if (el) {
...
(function () {
    ...
    var Loader = function () {
        ...
        _createClass(Loader, [{
            ...
            value: function _loadTiles() {
                ...
                var _this2 = this;
                // Extract URL parts
                let urlParts = window.location.pathname.split('/').filter(Boolean); // Remove empty parts

                // Extract room type (first segment) and room ID (last segment)
                let roomType = urlParts[0]; // Gets "room2d" or "panorama"
                let roomId = urlParts.pop(); // Gets the ID
        ...
        function CheckboxFilter(options, tiles, filterTilesList, index) {
            ...
            if (Array.isArray(values) && values.length > 0) {
                _this8._items = values.map(function (value) {
                    return { value: value.trim(), checked: false };
                });
                ...
                this._tiles.forEach(function (tile) {
                    ...
                    if (tile.surface === _this11.surface) {
                        if (_this11.field === 'size') {
                            ...
                            tile[_this11.field] = tile.height + 'x' + tile.width; //Added by Kinjal-TPS
                        }
    ...
    var Filters = function () {
        function Filters(tiles, room, onLoad) {
            ...
            this.additionalSearchFields = String(JsConstants.config.ProductInfo.additionalSearchFields || '').split(','); //Added by Kinjal-TPS
            ...
            this._defaultFilters.push({
                ...
                name: 'Size', field: 'size', surface: 'floor', type: 'checkbox'

            });

            this._defaultFilters.push({

                name: 'Size', field: 'size', surface: 'wall', type: 'checkbox'
        ...
        //Ends by Kinjal-TPS
        {
            ...
            value: function _filterTilesList() {
                ...
                var matchedTiles = []; //Added By Kinjal-TPS
                ...
                var sortedTiles = _this33._sortTilesByBrandPriority(matchedTiles);
        ...
        },
        {
            ...
            value: function _filterMatch(tile) {
                ...
                var match = this._list.filter(function (filter) {
                    ...
                    var count = checkAllBlankCategories(filter.field, _this21._room.currentTiledSurface.type);
    ...
    function Tile(tileData, index) {
        ...
        this.brand = tileData.brand;
        this.sub_brand = tileData.sub_brand_1;
    ...
    (function () {
        ...
        this.getInfoText = function () {
            ...
            var size = config.size ? '<p>' + Locale.lang('Size') + ': ' + this.height + 'mm x ' + this.width + 'mm</p>' : '';
        ...
        this.createTileLi = function () {
            ...
            this.li.dataset.tile = this.id; // Adding data-tile attribute
            this.li.dataset.surface = this.surface; // Adding data-surface attribute
            ...
            this.li.finalize = () => {
                ...
                $(document).on('click', '#tile_' + this.index, function () {
                    getTileId(this.id);
                });
    ...
    function UI(room) {
        ...
        this.updateValues = function (surface) {
            ...
            console.log("updateValues = " + surface.rotation);
        ...
        function share(event) {
            ...
            var roomUrl = window.location.origin + currentRoom.image; //Change by Kinjal
...
UI.updateCanvasStyle = function (title) {
    // Ensure the canvas is retrieved
    if (!UI.roomCanvas) {
        UI.roomCanvas = containerDiv.getElementsByTagName("canvas")[0]; // Try to get it dynamically
        if (!UI.roomCanvas) {
            console.warn("roomCanvas element not found!");
            return; // Exit if still not found
        }
    }

    // Update title and cursor style if title is a valid string
    if (title && typeof title === 'string') {
        UI.roomCanvas.title = 'Change ' + title;
        UI.roomCanvas.style.cursor = 'url("/img/icons/brush.png") 18 46, pointer';

        // Add a class to roomCanvas when title is a valid string
        UI.roomCanvas.classList.add('active-canvas'); // Example class added
    } else {
        // Reset title and cursor if title is invalid or empty
        UI.roomCanvas.title = '';
        UI.roomCanvas.style.cursor = 'unset';

        // Optionally remove a class when title is invalid
        UI.roomCanvas.classList.remove('active-canvas');
    }

    // Get the title from the UI.roomCanvas and split it into words
    var titleWords = UI.roomCanvas.title.split(' ');  // Split the title by spaces
    var firstTwoWords = titleWords.slice(0, 2).join(' ');  // Take the first two words and join them with a space

    // Check if the first two words of the title are "change paint"
    ...
    // Check if the first two words of the title are "change counter"
    if (firstTwoWords.toLowerCase() === "change counter") {
        $('#btnGrout').hide();  // Hide the grout button
    } else {
        $('#btnGrout').show();  // Show the grout button
    }
};

    //Updated By Kinjal ends
    /**
                    ...
                    if (saveIndexes === true) {
                        ...
                        url.forEach(function (url) {
                            ...
                            if (url && typeof url === 'string') {
                                ...
                                image.onload = function () {
                                    loadedCount += 1;
                                    console.log("IMAGE ARRAY ON LOAD");
                        ...
                        url.forEach(function (url) {
                            if (url && typeof url === 'string') {
                                ...
                                _image.onload = function () {
                                    console.log("IMAGE STRING ON LOAD");
                                    images.push(_image);
    ...
    var Room = function () {
        ...
        _createClass(Room, [{
            ...
            value: function _setPatternRotation(event) {
                console.log("KEY _setPatternRotation");
                if (!this._isSurfaceSelected()) return;
            ...
            value: function _onTileClick(tileIndex, secondLayer) {
                ...
                if (window.innerWidth < 767 && window.innerHeight > window.innerWidth) {
                    ...
                    document.getElementById('hideForPanorama').style.display = 'none';
                }
                //Ends by Kinjal-TPS
            ...
            value: function _initElements() {
                ...
                $('.radio-surface-rotation > input').click(function (event) {
                    console.log('.radio-surface-rotation > input');
                ...
                //CODE BY TPS TO START ROTATION BUTTON WORKING
    ...
    var CubeView = function () {
        ...
        _createClass(CubeView, [{
            ...
            key: '_updateTheme',
            value: function _updateTheme(textureId) {
                ...
                value: function _getMaterials(textures) {
                    ...
                    for (var i = 0; i < 6; i += 1) {
                        ...
                        if (textures) {
                            ...
                            console.log("IN IIII = " + i);
                            console.log(textures.images[i]);
    ...
    var GLOSSY_MASK_MATERIAL = new THREE.MeshBasicMaterial({
        color: 0x000000,
        polygonOffset: true,
        polygonOffsetFactor: -1,
        // stencilWrite: true,
    ...
    var PanoramaTiledSurface = function (_TiledSurface) {
        ...
        function PanoramaTiledSurface(surfaceData, geometry, updateMask, stencilRef, render) {
            ...
            _this47._surfaceData = surfaceData; //Added BY TPS
            _this47._updateMaskFunc = updateMask;

            this._foreground = null;
            this._themeImageURL = null;

            _this47._render = render || function () { console.log("Render function not defined"); };
        ...
        _createClass(PanoramaTiledSurface, [{
            ...
            value: function _createMesh(geometry) {
                ...
                this._invisibleMesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
                    color: 0xff5555,
                    colorWrite: false,
                    stencilWrite: true,
                    stencilRef: this._stencilRef,
    ...
    var PanoramaView = function (_View) {
        ...
        function PanoramaView(containerId, roomData, roomSettings) {
            ...
            this._render = this._render.bind(this); // Ensure the render method is bound to the current instance
        ...
        _createClass(PanoramaView, [{
            key: '_render',
            value: function _render(json = null) {
                // Rendering logic
...
//CODE BY TPS
            {
                ...
                value: function loadAndDrawForegroundImage(p_id) {
                    //if (this._themeImageURL == bigImage) return;
                    console.log("Hiding UI panels and showing loading animation...");
                    ...
                    /*for(var i=0;i<currentRoomMaterial.length;i++){
                        var materialObj = currentRoomMaterial[i];
                        console.log("materialObj i " + i);
                        console.log(materialObj);

                        var materialObj

                        console.log("T Image");
                        console.log(materialObj.uniforms.tImage);

                        //currentRoomMaterial[i].uniforms.tImage = "https://img.etimg.com/thumb/msid-57187317,width-640,height-480,imgsize-52484,resizemode-4/cool-things-youll-get-if-youre-a-google-employee.jpg";
                    ...
                    for(var j=0;j<currentRoomMaterial.length;j++){
                        console.log("roomTextures.theme p_id = " + p_id);
                        console.log("currentRoomMaterial J = " + j);
                        console.log(currentRoomMaterial[j]);

                        currentRoomMaterial[j].uniforms.tImage = { value: roomTextures["theme"+p_id]};
                        currentRoomMaterial[j].map = roomTextures["theme"+p_id];
                        currentRoomMaterial[j].needsUpdate = true;
                    ...
                    /* new THREE.TextureLoader().load("https://somany.easytrials.in/storage/panoramas/yjnxYQS3nvotpXnMCuRDbGORaVFcsxRNAXIkzqOG.jpg", function (texture) {


                         console.log("INSIDE LOAD TEXTURE LOAD")

                         for(var j=0;j<currentRoomMaterial.length;j++){
                             currentRoomMaterial[j].uniforms.tImage = texture;
                             currentRoomMaterial[j].map = texture;
                             currentRoomMaterial[j].needsUpdate = true;
                             loadingAnimation.hide();
                         }
                     });*/
                    ...
                    //image.onload = function () {
                    // if (tempThis._render) {
                    //       tempThis._render(json);  // Pass valid data
                    //       loadingAnimation.hide();
                    //  }
                    //};
                    //image.src = bigImage;
...

            {
                ...
                value: function _getTexturesFromAtlasFile(url, onLoad) {
                    ...
                    image.onload = function () {
                        ...
                        textures.forEach(function (texture, i) {
                            ...
                            canvas.height = size;
                            var context = canvas.getContext('2d');
                            console.log("Texture Draw Image");
                            console.log(image);
                        ...
                        console.log(RoomLoadProgress.itemsLoaded);
                ...
                value: function _createCubeView(textureUrls, onCreated) {
                    ...
                    function onTextureLoad(size) {
                        ...
                        _this._scene.add(_this._cubeView.camera);
                        _this.render();
                        //currentCubeView = _this._cubeView;
                    }
                    ...
                    //CODE BY TPS - FOR THEME

                    textures.theme0 = this._getTexturesFromAtlasFile(textureUrls.image);
                    if(this._getTexturesFromAtlasFile(textureUrls.theme1))
                        textures.theme1 = this._getTexturesFromAtlasFile(textureUrls.theme1);
                    if(this._getTexturesFromAtlasFile(textureUrls.theme2))
                        textures.theme2 = this._getTexturesFromAtlasFile(textureUrls.theme2);
                    if(this._getTexturesFromAtlasFile(textureUrls.theme3))
                        textures.theme3 = this._getTexturesFromAtlasFile(textureUrls.theme3);
                    if(this._getTexturesFromAtlasFile(textureUrls.theme4))
                        textures.theme4 = this._getTexturesFromAtlasFile(textureUrls.theme4);
                    if(this._getTexturesFromAtlasFile(textureUrls.theme5))
                        textures.theme5 = this._getTexturesFromAtlasFile(textureUrls.theme5);
                    textures.currentTexture = -1;
                    ...
                    if (surfacesData && typeof surfacesData === 'string') {
                        ...
                        if (Array.isArray(surfaces)) {
                            ...
                            surfaces.forEach(function (surface, i) {
                                //CODE BY TPS
                                surface.custom_surface_name = getCustomNameOfSurfaceData(surface["type"]);
                                //END
                ...
                value: function _onMouseMove(event) {
                    ...
                    var mouseOverSurface = intersects.some(function (intersect) {
                        if (intersect.object.surface instanceof PanoramaTiledSurface) {
                            if (_this._backLightedSurface !== intersect.object.surface) {
                                _this._clearBackLight();
                                _this._backLightedSurface = intersect.object.surface;
                                _this._backLightedSurface.backLight();
                            }

                            // Update brush icon and title when hovering over a surface
                            UI.updateCanvasStyle(intersect.object.surface._surfaceData.custom_surface_name);
                            return true;
                        }
                    });

                    if (!mouseOverSurface) {
                        this._clearBackLight();
                        // Reset brush icon when not hovering over a surface
    ...
    var PanoramaRoom = function (_Room) {
        ...
        _createClass(PanoramaRoom, [{
            ...
            value: function _createView(data) {
                ...
                console.log("TILED SURFACES");
                console.log(this.tiledSurfaces);
    ...
    console.log("Call Panorama Load Function");
    window.addEventListener('load', function () {
        new Loader().loadRoom('/get/panorama-studio/', function (data) {
            currentRoom = new PanoramaRoom(data);
