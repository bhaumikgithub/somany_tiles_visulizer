...
// if ( window.JsConstants.config.socketServer === "somany") {
//   var ws = new WebSocket("wss://162.246.16.14:8061");
// } else {
...
(function () {
  ...
  var isPanelVisible = false; // State variable
  var isSearchPanelVisible = false; // State variable

  $(document).on('click', '#sliderIconToggle', function () {
      var $panel = $('.filterContentPanel');
      var $currentButton = $(this);

      if (isPanelVisible) {
          $panel.hide();  // Hide the panel
          $currentButton.removeClass('top-panel-button-active');
          isPanelVisible = false;  // Update the state
          console.log('Panel hidden');
      } else {
          $panel.show();  // Show the panel
          $currentButton.addClass('top-panel-button-active');
          isPanelVisible = true;  // Update the state
          console.log('Panel shown');
      }
  });

  $(document).on('click', '#searchIconToggle', function () {
    var $panel = $('.serach-pad-set'); // Correct class name
    var $currentButton = $(this);

    if (isSearchPanelVisible) {
        $panel.hide();  // Hide the panel
        $currentButton.removeClass('top-panel-button-active');
        isSearchPanelVisible = false;  // Update the state
        console.log('Search panel hidden');
    } else {
        $panel.show();  // Show the panel
        $currentButton.addClass('top-panel-button-active');
        isSearchPanelVisible = true;  // Update the state
...
$('#topPanelHideIcon').on('click', function () {
  if (isMobileOrPortrait()) {
      ...
      if ($(window).width() < 991 && isLandscape()) {
          ...
          if ($icon.hasClass('glyphicon') && $icon.hasClass('glyphicon-menu-left')) {
             ...
             if ($('#sliderIconToggle').hasClass('top-panel-button-active')) {
               $('.filterContentPanel').show();
              
           }
           if ($('#searchIconToggle').hasClass('top-panel-button-active')) {
             $('.serach-pad-set').show();
         }
             ...
             $('#topPanelTilesListBox').css({ 'display': 'block' });
           $('.serach-pad-set, .partOfProductTabContent , filterContentPanel ').hide();
...
function landscapadjustPanelPosition() {
  ...
  var windowHeight = $(window).height();
   ...
   $mainPanel.css({
      ...
      'width': '350px',           // Set width to 300px
      'right': '0px' ,          // Shift the panel 300px to the left (off-screen)
  ...
  if ($(window).width() > 991)
    {
      $mainPanel.css({
        ...
        'top': '0px',              // 10px from the top
        'height': (windowHeight ) + 'px', // Full viewport height minus 20px for spacing
        'width': '450px',           // Set width to 300px
        'right': '0px' ,          // Shift the panel 300px to the left (off-screen)
        'border-top-right-radius': '0px' 
    });
...
var isRoomCanvasClicked = false; // Flag to track click status

$('#roomCanvas').on('click', function () {
  isRoomCanvasClicked = true;
  if (isFirstClick) {
...
if (isRoomCanvasClicked) {
          ...
          this._tiles.forEach(function (tile) {
            ...
            if (tile.surface === _this18.surface) {
              if (_this18.field === "size") {
                ...
                tile[_this18.field] = tile.height + "x" + tile.width; //Change by Kinjal-TPS
              }
  ...
  function productInfoAssText(tile, surfaceName, surfaceIndex) {
    ...
    var size = config.size
      ? Locale.lang("Size") + ": " + tile.height + "mm x " + tile.width + "mm\n"
      : "";
        ...
        value: function _onTileClick(tileIndex, secondLayer) {
          ...
          if (surface.freeDesign) {
            ...
            TilesList.updateLayerButtons(tileIndex);
			      ...
			      /**** Code Started by Kinjal *****/
            //Get the grout size element
            let groutTextElement = document.getElementById("topPanelGroutSizeText");
            let previousGroutSize = surface.groutSize; // Store the current grout size before applying tile

            let currentGroutSize = groutTextElement 
              ? parseInt(groutTextElement.innerText.trim(), 10) || 0 
              : previousGroutSize; // Use previous grout size if no valid number found

            console.log("Previous Grout Size:", previousGroutSize);
            console.log("Extracted Grout Size:", currentGroutSize);

            // If auto-set to 0, reset to 4 mm (but allow user-set 0 to remain)
            if (currentGroutSize === 0 && groutTextElement.innerText.trim() !== "0 mm") {
              currentGroutSize = previousGroutSize > 0 ? previousGroutSize : 4; // Keep previous grout size or default to 4mm
              console.log("Grout size restored to:", currentGroutSize);
            }

            // Update the surface's grout size before applying
            surface.groutSize = currentGroutSize;

            // Pass the corrected value to the function
            this._ui.changeGroutSize(currentGroutSize);

            /**** Code Ends by Kinjal E *****/
          }
