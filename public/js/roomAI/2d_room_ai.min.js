...
(function () {
  ...
  //   $(document).on('click', '#sliderIconToggle', function (event) {
  ...
  //         if ($('.search-pad-set').is(':visible')) {
  //             $('.search-pad-set').hide();
  //             $('#searchIconToggle').removeClass('top-panel-button-active');
  //         }
...
$('#topPanelHideIcon').on('click', function () {
  ...
  var containerBottom = $roomCanvasContainer.offset().top + $roomCanvasContainer.outerHeight();
  var windowHeight = $(window).height();
  var topPosition = windowHeight - 30;

  // Handle mobile/portrait layout
  if ($(window).width() < 767 && isLandscape()) {
      if ($icon.hasClass('glyphicon-menu-left')) {
          $hideBtn.css({ 'top': '20px' });
          $mainPanel.css({
              'position': 'fixed',
              'top': '0px',
  ...
  // **Important:** Only hide panels if they were previously shown
  if (isPanelVisible) {
      $('.filterContentPanel').hide();
      isPanelVisible = false; // Update state
  }
  if (isSearchPanelVisible) {
      $('.serach-pad-set').hide();
      isSearchPanelVisible = false; // Update state
  }
});
