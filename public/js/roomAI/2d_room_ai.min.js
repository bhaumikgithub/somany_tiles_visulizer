...
var maxReconnects = 5; // Limit reconnection attempts
var reconnectDelay = 5000; // Start with a 5-second delay
/**
...
(function () {
    ...
    function UI(room) {
        ...
        this.topPanelSlide = function (show) {
            ...
            if (show === true) {
                ...
                //CODE BY TPS - CALLING CUSTOM.JS
                openTopPanel();
                //END

                // }

            } else { //else if (topPanelVisible) {
        ...
        function getProductInfoElement(tile) {
            ...
            //console.log("tile.file = " + tile.file);
        ...
        function init() {
            ...
            $('#topPanelHideBtn').click(function () {
                    //CODE BY TPS
                    showMainInfoPanel("MAINLISTING_SHOW");
                    _this.topPanelToggle();
                    //END
                }
            );
        ...
        _createClass(LoadingAnimation, [{
            ...
            value: function show(p_msg) {
                ...
                if (!this.preLoader) {
                    ...
                    if (p_msg)
                        $("#loadAnimationContainer").find("p").html(p_msg);
                    else
                        $("#loadAnimationContainer").find("p").html("Applying Tiles");
    ...
    var Loader = function () {
        ...
        _createClass(Loader, [{
            ...
            value: function _loadTiles() {
                ...
                var roomId = urlParts.pop(); // Gets the ID
                // Validate room ID (ensure it's a number)
                if (!isNaN(roomId) && roomId !== undefined) {
                    $.ajax({
                        url: '/get/tiles',
                        type: 'GET',
                        data: {
            ...
            value: function _onWholeDataLoaded(part, data) {
                ...
                if (wholeDataLoaded && typeof this.onLoad === 'function') {
                    //CHANGES BY TPS
                    /*
                    var surfaceJSON = $.parseJSON(this.data.roomData["surfaces"]);
                    var alphabets = ["A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P"];

                    var roomSurfaceData = [];
                    for(var i = 0;i<surfaceJSON.length;i++){
                        var iSurfaceType = surfaceJSON[i]["176"]; //Wall, floor etc

                        var surfaceTypeFoundCount = 0;
                        for(var j=0;j<roomSurfaceData.length;j++){
                            if(roomSurfaceData[j].surfaceType == iSurfaceType){
                                surfaceTypeFoundCount++;
                            }
                        }
                        var obj = {};
                        obj.surfaceType = iSurfaceType;
                        obj.surfaceGroup = surfaceJSON[i]["group"];
                        obj.surfaceName = iSurfaceType + " " + alphabets[surfaceTypeFoundCount];
                        obj.surfaceAlphabet = alphabets[surfaceTypeFoundCount];
                        obj.surfaceData = surfaceJSON[i];
                        roomSurfaceData.push(obj);
            ...
            value: function match(tile, surface) {
                ...
                if (surface && surface.freeDesign) {
                    ...
                    if (currentTile) {
                        ...
                        if (Array.isArray(currentTile.categories) && currentTile.categories.length !== 0) {

                            return currentTile.categories[0] === (tile.categories && tile.categories[0]);
            ...
            value: function _filter() {
                if (typeof this._filterCallback === 'function') {
                    ...
                    if (this._activeTab === this._tabAll) {
                        ...
                        this._activeTabIds = [this._activeTab.id].concat(_toConsumableArray(activeCheckBoxes));
        ...
        _createClass(Filter, [{
            ...
            value: function _apply() {
                ...
                if (typeof this._filterTilesList === 'function') {
                    console.log("filter class inside");
    ...
    var CheckboxFilter = function (_Filter) {
        ...
        function CheckboxFilter(options, tiles, filterTilesList, index) {
            ...
            var _this15 = _possibleConstructorReturn(this, (CheckboxFilter.__proto__ || Object.getPrototypeOf(CheckboxFilter)).call(this, options, tiles, filterTilesList));
            ...
            //var checkboxClassName = options.surface + "_" + options.name;
            //END
        ...
        _createClass(CheckboxFilter, [{
            ...
            value: function match(fieldValue) {
                ...
                if (Array.isArray(this._items)) {
                    ...
                    return this._items.some(function (item) {
                        ...
                        if (item.value === fieldValue || _this16._subItemMatch(item.value, fieldValue)) {
            ...
            value: function getValues() {
                ...
                var values = this._items.filter(function (item) {
            ...
            value: function _parseValues() {
                ...
                var addValue = function addValue(text) {
                    var item = typeof text === 'string' ? _this18._getCheckboxFilterItem(text.trim(), text.trim().toLowerCase()) : _this18._getCheckboxFilterItem(String(text), text);

                    if ((item.value || Number(item.value) === 0) && !items.some(function (i) {
            ...
            value: function _getCheckboxFilterItem(name, value) {
                return { checked: false, domElement: undefined, name: name, value: value };
    ...
    var AccordionFilter = function (_CheckboxFilter) {
        ...
        function AccordionFilter(options, tiles, filterTilesList, index, updateAllAccordionFilters) {

            _classCallCheck(this, AccordionFilter);



            var _this20 = _possibleConstructorReturn(this, (AccordionFilter.__proto__ || Object.getPrototypeOf(AccordionFilter)).call(this, options, tiles, filterTilesList, index));
        ...
        _createClass(AccordionFilter, [{
            ...
            value: function createDomElement() {
                ...
                this.domElement = document.getElementsByClassName('accordion-body')[0];

                this._accordionHeader = document.getElementsByClassName('accordion-header')[0];
            ...
            value: function _getCheckboxFilterItem(name, value) {
                ...
                return { checked: SHOW_ALL_UNFILTERED_PRODUCT, domElement: undefined, name: name, value: value };
    ...
    var SliderFilter = function (_Filter2) {
        ...
        function SliderFilter(options, tiles, filterTilesList) {
            ...
            var _this25 = _possibleConstructorReturn(this, (SliderFilter.__proto__ || Object.getPrototypeOf(SliderFilter)).call(this, options, tiles, filterTilesList));
        ...
        _createClass(SliderFilter, [{
            ...
            value: function _updateHeader(min, max) {
                ...
                this._title.innerHTML = this.field === 'price' ? Locale.lang(this.field) + ': $' + min + ' - $' + max : Locale.lang(this.field) + ': ' + min + ' - ' + max;
    ...
    var Filters = function () {
        ...
        function Filters(tiles, room, onLoad) {
            ...
            this.additionalSearchFields = String(JsConstants.config.ProductInfo.additionalSearchFields || '').split(',');

            if (JsConstants.config.productFilterSize) {

                this._defaultFilters.push({

                    name: 'Size', field: 'size', surface: SurfaceType.Floor, type: FilterType.Checkbox

                });

                this._defaultFilters.push({

                    name: 'Size', field: 'size', surface: SurfaceType.Wall, type: FilterType.Checkbox
            ...
            //     this._defaultFilters.push({
            //
            //         name: 'Finish', field: 'finish', surface: SurfaceType.Floor, type: FilterType.Checkbox
            //
            //     });
            //
            //     this._defaultFilters.push({
            //
            //         name: 'Finish', field: 'finish', surface: SurfaceType.Wall, type: FilterType.Checkbox
            //
            //     });
            //
            // }
            ...
            // if (JsConstants.config.ProductInfo.colors) {
            //
            //     this._defaultFilters.push({
            //
            //         name: 'Colours', field: 'colour', surface: SurfaceType.Floor, type: FilterType.Checkbox
            //
            //     });
            //
            //     this._defaultFilters.push({
            //
            //         name: 'Colours', field: 'colour', surface: SurfaceType.Wall, type: FilterType.Checkbox
            //
            //     });
            //
            // }
            ...
            var accordionFilter = document.getElementsByClassName('accordion-filter')[0];
            ...
            if (accordionFilter) {
                ...
                window.addEventListener('click', function (e) {
                    ...
                    if (e) {
                        ...
                        if (Array.isArray(path) && path.some(function (element) {
        ...
        _createClass(Filters, [{
            ...
            value: function show() {
                ...
                if (this._loaded && this._room.currentTiledSurface) {
                    ...
                    console.log("filter insdie 4511");
                    this._filterTilesList();
            ...
            value: function _find() {
                ...
                this._tiles.forEach(function (tile) {
                    ...
                    if (tile.li.hidedByFilter || tile.li.rotoHidden || tile.li.dataset.hidden === 'hidden' || tile.removed) {
            ...
            value: function _matchFields(tile) {
                ...
                var category = tile.category ? tile.category.toLowerCase() : '';

                if (name.indexOf(this._searchWord) !== -1 || category.indexOf(this._searchWord) !== -1) {
                    return true;
                }
                ...
                return this.additionalSearchFields.some(function (field) {
                    ...
                    if (typeof fieldText === 'string') {
                        ...
                        return fieldText.trim().toLowerCase().indexOf(_this31._searchWord) !== -1;
            ...
            value: function _init(loadedFilters) {
                ...
                console.log(loadedFilters);
                console.log("this._defaultFilters");
                console.log(this._defaultFilters);
                filtersList.forEach(function (filterOptions, index) {
                    ...
                    switch (filterOptions.type) {
                        ...
                        case FilterType.Checkbox:
                            ...
                            filter = new CheckboxFilter(filterOptions, _this32._tiles, function () {
                        ...
                        case FilterType.Slider:
                            ...
                            filter = new SliderFilter(filterOptions, _this32._tiles, function () {
                        ...
                        case FilterType.Accordion:
                            ...
                            filter = new AccordionFilter(filterOptions, _this32._tiles, function () {
                ...
                if (this._list.some(function (filter) {
            ...
            value: function _addAccordionFilter(filter) {
                ...
                var topPanelFilter = document.getElementsByClassName('filter-accordion');
            ...
            {
                key: '_filterShowAllTiles',

                value: function _filterShowAllTiles() {
            ...
            // }
            {
                ...
                value: function _filterTilesList() {
                    ...
                    var forceSelectAll = totalFilterCheckboxesChecked() == 0;
                    var addedTiles = new Set(); // Prevent duplicate tiles
                    var visibleWallSizes = new Set(); // Track final visible Wall sizes
                    var visibleFloorSizes = new Set(); // Track final visible Floor sizes
                    ...
                    this._tiles.forEach(function (tile) {
                        ...
                        var correctSurface = !tile.surface || tile.surface === _this33._room.currentTiledSurface.type;
                        let match = _this33._filterMatch(tile);
                        ...
                        var roomType = _this33._room.type.toLowerCase();
                        var surfaceType = tile.surface ? tile.surface.toLowerCase() : "";
                        var tileKey = `${tile.name}-${tile.size}-${surfaceType}`;

                        // Identify if the tile is a Counter tile
                        var isCounterTile = _this33._tiles.some(t => t.name === tile.name && t.surface === "counter");

                        // Hide Wall & Floor tiles in Kitchen/Bathroom if a Counter tile exists
                        var hideWallTile = ["kitchen", "bathroom"].includes(roomType) && surfaceType === "wall" && isCounterTile;
                        var hideFloorTile = ["kitchen", "bathroom"].includes(roomType) && surfaceType === "floor" && isCounterTile;

                        if (hideWallTile || hideFloorTile) {
                            tile.hide();
                            tile.li.hidedByFilter = true;
                            return;
                        }

                        // Allow Wall & Floor tiles for other rooms (Lobby, Reception, etc.)
                        var allowAllSurfaces = ["lobby", "reception", "mall", "restaurant", "hotels"].includes(roomType);
                        if (allowAllSurfaces) {
                            tile.show();
                        ...
                        // Prevent duplicate tiles on Counter
                        var isDuplicateCounterTile = surfaceType === "counter" && addedTiles.has(tileKey);
                        if (isDuplicateCounterTile) {
                            console.warn(`Duplicate removed: ${tile.name} on counter`);
                            return;
                        }

                        //Add only visible Wall & Floor tile sizes (Exclude Counter-only tiles)
                        if ((correctSurface && match && matchCategory) || tile.custom) {
                            tile.show();
                            ...
                            // Ensure Counter tiles do not contribute to Wall/Floor size filters
                            if (surfaceType === "wall") {
                                visibleWallSizes.add(tile.width + "x" + tile.height);
                            }
                            if (surfaceType === "floor") {
                                visibleFloorSizes.add(tile.width + "x" + tile.height);
                            }
                        } else {
                            tile.hide();
                            if (!tile.li.hidedByFilter) {
                    ...
                    // Update size filter based on final visible Wall/Floor tiles
                    this._updateSizeFilter(visibleWallSizes, visibleFloorSizes);
            ...
            {
                key: '_updateSizeFilter',
                value: function _updateSizeFilter(originalWallSizes, originalFloorSizes ) {
                    var _this33 = this; // Store reference to `this`
                    var allValidSizes = new Set([...originalWallSizes, ...originalFloorSizes]); // Store all valid sizes

                    var sizeFilters = document.querySelectorAll('.filter-item-checkbox'); // Select size-related filters

                    sizeFilters.forEach(function (filterItem) {
                        var checkbox = filterItem.querySelector("input.checkboxClass");
                        var label = filterItem.querySelector("label");

                        if (!checkbox || !label) {
                            console.warn(`Skipping: Checkbox or label not found`);
                            return;
                        }

                        var labelText = label.textContent.trim();
                        var sizeValueForMatching = labelText.replace("mm", ""); // Remove "mm" for filtering

                        var parentElement = filterItem.closest("li") || filterItem;

                        // Ensure only size labels are filtered
                        var isSizeLabel = /^\d+x\d+$/.test(sizeValueForMatching); // Matches "600x1200" but not "FULL POLISHED"
                        if (!isSizeLabel) {
                            parentElement.style.display = "flex"; // Keep finish labels visible
                            return;
                        }

                        // ✅ Ensure Counter sizes are removed from Wall/Floor but stay in Counter
                        var isCounterFilter = parentElement.closest("#-body_counter_size"); // Check if it's under Counter filter
                        if (isCounterFilter) {
                            parentElement.style.display = "flex";
                            return;
                        }

                        // Ensure all originally valid Wall & Floor sizes remain
                        if (originalWallSizes.has(sizeValueForMatching) || originalFloorSizes.has(sizeValueForMatching)) {
                            parentElement.style.display = "flex";
                        } else {
                            parentElement.style.display = "none";
                        }

                        // Ensure filtering tiles does not modify `Valid Wall Sizes`
                        checkbox.addEventListener("change", function () {
                            sizeFilters.forEach(function (item) {
                                var itemLabel = item.querySelector("label");
                                if (itemLabel) {
                                    var itemSize = itemLabel.textContent.trim().replace("mm", "");
                                    if (allValidSizes.has(itemSize)) {
                                        item.style.display = "flex"; // Show valid sizes
            ...
            , {
                ...
                value: function _applyUrlFilters() {
                    ...
                    this._searchWord = filters.searchWord ? filters.searchWord.toString().trim().toLowerCase() : '';
                    ...
                    this._list.forEach(function (filterItem) {
                        ...
                        var filter = filters.items.find(function (item) {
                            ...
                            return filterItem.surface === item.surface && filterItem.field === item.field;
                ...
                value: function _updateUrlParams() {
                    ...
                    var filters = {
                        ...
                        items: this._list.map(function (item) {
    ...
    (function () {
        ...
        this.createTileLi = function () {
            ...
            var $li = $('<li id="tile_' + this.index + '" data-index="' + this.index + '"  data-tile="' + this.id + '" class="' + liClass + '" style="display: none;" onclick="getTileId(this.id)"></li>').click(selectTile).append($imageHolder, $tileText);
    ...
    function userRoom(room, roomSettings, engine, interiorUI) {
        ...
        var filters = window.JsConstants.config.urlFilters ? new URL(window.location.href).searchParams.get('filters') : undefined;
    ...
    function milimetersToPoints(milimeters) {
        ...
        var dpi = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : DPI;
    ...
    function productInfoAssText(tile, surfaceName, surfaceIndex) {
        ...
        return {
            ...
            text: size + shape + finish + price + url + category + extraOptions + usedColors + rotoPrintSet
    ...
    function saveDoc(room, tiles, docType, pdfAsImage) {
        ...
        Images.load(urls, function (tileImages) {
            ...
            var logoMaxWidth = window.JsConstants.config.pdfHeaderText ? PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT - 5 - headerWidth : PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT;
    ...
    function docVisualizer(room, tiles) {
        ...
        var logoMaxWidth = window.JsConstants.config.pdfHeaderText ? PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT - 5 - window.JsConstants.config.pdfHeaderTextWidth : PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT;

        var logoWidth = milimetersToPoints(Math.min(logoMaxWidth, window.JsConstants.config.pdfLogoMaxWidth), 72);
        ...
        Images.load(urls, function (tileImages) {
            ...
            var header = window.JsConstants.config.pdfHeaderText ? {
                ...
                columns: [{
                    ...
                    text: window.JsConstants.config.pdfHeaderText.replace(/,/g, '\n'),
            ...
            var doc = {
                ...
                content: [header, {
                    ...
                    fit: [milimetersToPoints(PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT, 72), 320],
            ...
            tilesInfo.forEach(function (tile, i) {
                ...
                if (lastSurfaceIndex !== tile.surfaceIndex) {
                    ...
                    doc.content.push([{
                        ...
                        width: milimetersToPoints(PAGE_WIDTH - MARGIN_LEFT - MARGIN_RIGHT, 72),
        ...
        _createClass(Room, [{
            ...
            value: function _share(event) {
                ...
                userRoom2(this, settings, this._engineType, this._ui, function () {
                    ...
                    return _this39._ui.share(event);
            ...
            value: function _onTileClick(tileIndex, secondLayer) {
                ...
                //CODE BY TPS
                clickedTiles(tile, surface._surfaceData.custom_surface_name);
                //END
                ...
                if (window.JsConstants.config.useProductCategory && this._filters) this._filters.show();
            ...
            value: function _onSurfaceClick(surface,forceOpen) {

                //CODE BY TPS
                var openPanel = false;
                //END
                var topPanel = document.getElementById('topPanel'); // TODO Fix for iorena
                ...
                if (rotate && (surface.tile || surface.tile2) || surface.freeDesign && surface.freeDesignTile) {
                    //if ( (surface.tile || surface.tile2) || surface.freeDesign && surface.freeDesignTile) {
                    openPanel = false;
                ...
                this._ui.hideProductInfoPanel();
                if(openPanel==true || forceOpen== true){
            ...
            value: function _fillTiles(data) {
                ...
                var onClick = function onClick(tileIndex, secondLayer) {
                    ...
                    return _this40._onTileClick(tileIndex, secondLayer);
                ...
                this._ui.filtersShow = function () {
                    ...
                    return _this40._filters.show();

                };
                console.log("_fillTiles");
            }
            ...
            value: function _initElements() {
                ...
                $('#topPanelCheckFreeDesign').change(function (event) {
                    ...
                    return _this41._setCheckFreeDesign(event);
                ...
                $('.radio-surface-pattern > input').change(function (event) {
                    ...
                    return _this41._setPattern(event);

                });

                $('.radio-surface-pattern > li').click(function (event) {
                ...
                $('.radio-skew-size input').change(function (event) {
                    ...
                    return _this41._setSkewSize(event);
                ...
                $("#topPanelSurfaceRotation_45").click(function (event) {
                    console.log("topPanelSurfaceRotation_45 CLICKED");
                    return _this41._setPatternRotation(event);
                });
                ...
                $('.radio-surface-rotation > input').click(function (event) {
                    console.log("_setPatternRotation input");
                    return _this41._setPatternRotation(event);

                });

                $('.radio-surface-rotation > li').click(function (event) {
                    console.log("_setPatternRotation li");
                    return _this41._setPatternRotation(event);
                ...
                if (window.JsConstants.config.saveImageAsDoc) {
                    ...
                    $('#btnDialogSaveImage').click(function () {
                        return imageAsDoc(_this41, TilesList);
                    ...
                    $('#btnDialogSaveImage').click(function () {
                        return canvasImage(_this41.canvas);
                ...
                $('#btnDialogSavePdf').click(function () {
                    ...
                    return saveRoomPdf(_this41, TilesList);

                });

                $('#btnDialogSaveScene').click(function () {

                    return _this41._save();
    ...
    var Engine2D = function () {
        ...
        _createClass(Engine2D, [{
            ...
            value: function _drawSurfaceCanvas(surface) {
                ...
                //console.log("Inside drawsurfacecanv")
            ...
            value: function _drawTiledShape(surface) {
                ...
                if (surface) {
                    ...
                    //console.log("calling _drawSurfaceCanvas 8933");
            ...
            value: function _render(surfaces) {
                ...
                //console.log("IN _render A | 8975");

                //console.log(this._foreground);
                //console.log(this._shadowMatt);
                //console.log(this._shadowGlossy);
                ...
                if (this._foreground && this._shadowMatt && this._shadowGlossy) {
                    ...
                    // this.canvasContext.drawImage(this._themeImage, 0, 0, this.width, this.height);
                    drawOnce = true;
                }
                if (this._foreground && drawOnce == false) {
            ...
            value: function _loadImage(imageUrl, onLoad, onProgress, setSize, surfaces) {
                ...
                if (imageUrl) {
                    ...
                    img.onload = function () {
                        ...
                        if (_this44._imagesLoadedCount >= _this44._imagesTotalCount) {
                            //console.log("Calling render from 9036 | _loadImage");
                        ...
                        if (onProgress) onProgress(_this44._imagesLoadedCount, _this44._imagesTotalCount);
            ...
            value: function render(surfaces, backLightSurfaces, cameraDirection) {
                ...
                //console.log("IN _render B | 9058 | ENGINE 2D");
            ...
            value: function loadImages(urls, onLoad, onProgress, surfaces) {
                ...
                if (this._themeImageURL) {
                    this._foreground = this._loadImage(this._themeImageURL, onLoad, onProgress, true, surfaces);
                }
                else {
                    this._foreground = this._loadImage(urls.image, onLoad, onProgress, true, surfaces);
                }
                if (urls.shadow)
                    var shadowGlossy = this._loadImage(urls.shadow, onLoad, onProgress, false, surfaces);

                if (urls.shadow_matt)
                    var shadowMatt = this._loadImage(urls.shadow_matt, onLoad, onProgress, false, surfaces);

                if (urls.shadow_matt || urls.shadow) {
            ...
            {
                key: 'loadAndDrawForegroundImage',
                value: function loadAndDrawForegroundImage(bigImage) {

                    if (this._themeImageURL == bigImage) return;

                    currentRoom._ui.topPanelHide();

                    var tempThis = this;
                    loadingAnimation.hide();
                    loadingAnimation.show("Applying Theme");

                    var image = new Image();

                    image.onload = function () {

                        tempThis._render(currentRoom.tiledSurfaces);
                        loadingAnimation.hide();
                        tempThis.canvasContext.drawImage(image, 0, 0, this.width, this.height);
                    };

                    image.src = bigImage;

                    this._foreground = image;
                    this._themeImageURL = bigImage;

                    //this.loadImages(this, RoomLoadProgress.end, RoomLoadProgress.progress, currentRoom.tiledSurfaces);
                    //setTimeout(function(){
                    //  console.log("render call LOAD AND DRAW");
                    //tempThis._render(currentRoom.tiledSurfaces);
                    //},5000);
                    //this._themeImage = this._loadImage(bigImage, onLoad, onProgress, true, surfaces);
                    //this.canvasContext.drawImage(this._themeImage, 0, 0, this.width, this.height);
                }
            },//END TPS CODE
            {
    ...
    var SURFACE_DEFAULTS = {
        ...
        fillTypeIndex: Number(window.JsConstants.config.TiledSurface.fillTypeIndex) || 0,
    ...
    var SurfaceProjection = function () {
        ...
        _createClass(SurfaceProjection, [{
            ...
            value: function render(layers, direction) {
                ...
                this._changeCameraDirection(direction);
                //console.log("IN _render D | 10257 | Surfaceprojection");
                //console.log("CALL RENDER 10258 FROM RENDER D");
                this._render();
            ...
            value: function _getRaycaster(point) {
                ...
                //console.log("Calling render from 10360 | Get ray");
                //console.log("CALL RENDER 10260 FROM GET RAY CASTEAR");
                this._render();
            ...
            value: function _render() {
                ...
                //console.log("IN _render B | 10370");
                //console.log("CALL RENDERER 10373 FROM RENDER B SURFACE PROJECTION");
    ...
    var MAX_TILED_PLANE_SIZE = 20000; // mm
    ...
    var TiledSurface2d = function (_TiledSurface) {
        ...
        function TiledSurface2d(surfaceData, render) {
            ...
            var _this46 = _possibleConstructorReturn(this, (TiledSurface2d.__proto__ || Object.getPrototypeOf(TiledSurface2d)).call(this, surfaceData));
        ...
        _createClass(TiledSurface2d, [{
            ...
            value: function _setData(data) {
                ...
                this.maxSize = MAX_TILED_PLANE_SIZE; // Math.min(Math.max(this.width, this.height), MAX_TILED_PLANE_SIZE);
            ...
            value: function _tilePositions() {
                ...
                this._getTilePosition = {
                    ...
                    square: function square(options, tile) {
                        ...
                        if (fillTypes[_this48.fillTypeIndex].name === 'Horizontal Skew' && _this48._isOdd(options.yIndex)) {
                        ...
                        if (fillTypes[_this48.fillTypeIndex].name === 'Vertical Skew' && _this48._isOdd(options.xIndex)) {
                    ...
                    diamond: function diamond(options, tile) {
                        ...
                        var y = options.yIndex * (tile.height + _this48.groutSize - tile.height / 2);
                    ...
                    preparedSet: function preparedSet(options, tile) {
                        ...
                        var x = options.xIndex * (tile.width + _this48.groutSize - tile.preparedSetOffset.x);

                        var y = options.yIndex * (tile.height + _this48.groutSize - tile.preparedSetOffset.y);
                    ...
                    quadSet: function quadSet(options, tile) {
                        ...
                        if (_this48._isOdd(options.xIndex) && _this48._isEven(options.yIndex)) {

                            rotation = ROTATION_270_DEG;

                        } else if (_this48._isEven(options.xIndex) && _this48._isOdd(options.yIndex)) {

                            rotation = ROTATION_90_DEG;

                        } else if (_this48._isOdd(options.xIndex) && _this48._isOdd(options.yIndex)) {
            ...
            value: function _drawTiles(tile, drawEver) {
                ...
                tilesSet.forEach(function (tile) {
                    ...
                    loadingAnimation.show();
                    document.getElementById('topPanelSearchResult').style.display = 'block';
                    ...
                    image.onload = function () {
                        ...
                        if (loadedImages >= tilesSet.length) {
                            ...
                            //console.log("CALL RENDER 11227 FROM DRAW TILES");
                            _this49._render();
                        ...
                        if (window.innerWidth < 767 && window.innerHeight > window.innerWidth) {
                            document.getElementById('topPanelSearchResult').style.display = 'none';
                        }
            ...
            //CODE BY TPS
            {
                ...
                value: function _(_drawTheme, drawEver) {
                    ...
                    tilesSet.forEach(function (tile) {
                        ...
                        image.onload = function () {
                            ...
                            if (loadedImages >= tilesSet.length) {
                                ...
                                _this49._fillSurface(tilesSet, drawEver);
                                //console.log("CALL RENDER 11289 FROM LOAD");
                                _this49._render();
            ...
            //TPS CODE END
            {
                ...
                value: function setColor() {
                    ...
                    this.color = color;
                    //console.log("CALL RENDER 11321 FROM SET COLOR");
                    this._render();
                ...
                value: function setFillType(value) {
                    ...
                    this.fillTypeIndex = Number(value) || 0;
                    //console.log("Draw - Set Fill Type");
                ...
                value: function setGroutColor(color) {
                    ...
                    //console.log("CALL RENDER 11397 FROM SETGROUTCOLOR");
                    this._render();
                ...
                value: function rotate(angle) {
                    ...
                    this._rotate(angle);
                    //console.log("CALL RENDER 11434 FROM ROTATE");
                    this._render();
                ...
                value: function _checkTilePlace(mesh) {
                    //console.log("CALL RENDER 11462 FROM CHECK TILE PLACE");
                ...
                value: function setFreeDesign(value) {
                    ...
                    if (!this.freeDesign) {
                        ...
                        //console.log("CALL RENDER 11528 FROM SET FREE DESIGN");
                        this._render();
                ...
                value: function _freeDesignTileApproxPositions() {
                    ...
                    this._freeDesignTileApproxPosition = {
                        ...
                        square: function square(point, tile) {
                            ...
                            var xIndex = Math.floor(point.x / (tile.width + _this50.groutSize));

                            var yIndex = Math.floor(point.y / (tile.height + _this50.groutSize));
                        ...
                        hexagon: function hexagon(point, tile) {
                            ...
                            var yIndex = Math.floor(point.y / (tile.height * 0.5 + _this50.groutSize * 0.5));
                        ...
                        diamond: function diamond(point, tile) {
                            ...
                            var yIndex = Math.floor(point.y / (tile.height * 0.5 + _this50.groutSize));
                        ...
                        preparedSet: function preparedSet(point, tile) {
                            ...
                            var xIndex = Math.floor(point.x / (tile.width + _this50.groutSize - tile.preparedSetOffset.x));

                            var yIndex = Math.floor(point.y / (tile.height + _this50.groutSize - tile.preparedSetOffset.y));
                ...
                value: function _putFreeDesignTileToCell(tile, point) {
                    ...
                    neighborTileCells.some(function (neighborTileCell) {
                        ...
                        _this51._moveFreeDesignTile(mesh, tile, { xIndex: cell.xIndex + neighborTileCell[0], yIndex: cell.yIndex + neighborTileCell[1] });
                    ...
                    this.freeDesignTiles.forEach(function (freeDesignTile) {
                        ...
                        if (tile) {
                            ...
                            image.onload = function () {
                                ...
                                var mesh = _this52._addTileToContainer(tile, _this52.freeDesignTilesContainer, { x: 0, y: 0 });
                                ...
                                if (loadedImages >= _this52.freeDesignTiles.length) {
                                    //console.log("CALL RENDER 11749 FROM IMAGE ON LOAD");
                                    _this52._render();
                ...
                value: function _addFreeDesignTile(intersect) {
                    ...
                    if (this.freeDesignTile) {
                        ...
                        image.onload = function () {
                            ...
                            //console.log("Calling render from 11808 | add free design");
                            _this53._render();
                ...
                value: function _rotateTile(mesh) {
                    ...
                    if (angle !== 0) {
                        ...
                        //console.log("CALL RENDER 11826 ROTATE TILES");
                        this._render();
                ...
                value: function _removeFreeDesignTile(tileMesh) {
                    ...
                    //console.log("CALL RENDER 11848 FROM REMOVE FREE DESIGN TILE");
                    this._render();
                ...
                value: function draw() {
                    ...
                    //console.log("Draw - Inside Draw Function");
                    this.removeAllTiles();
                ...
                value: function restoreSettings(savedData) {
                    ...
                    if (typeof savedData.color !== 'undefined') this.color = savedData.color;

                    if (typeof savedData.groutSize !== 'undefined') this.groutSize = Number(savedData.groutSize);

                    if (typeof savedData.groutColor !== 'undefined') this.groutColor = savedData.groutColor;

                    if (typeof savedData.fillTypeIndex !== 'undefined') this.fillTypeIndex = Number(savedData.fillTypeIndex);

                    if (typeof savedData.tileId !== 'undefined') this.tileIndex = TilesList.getIndexById(savedData.tileId, savedData.tileCustom);

                    if (typeof savedData.tile2Id !== 'undefined') this.tile2Index = TilesList.getIndexById(savedData.tile2Id, savedData.tile2Custom);

                    if (typeof savedData.freeDesign !== 'undefined') this.freeDesign = savedData.freeDesign;

                    if (typeof savedData.freeDesignTiles !== 'undefined') this.freeDesignTiles = savedData.freeDesignTiles;

                    if (typeof savedData.skewSize !== 'undefined') this.skewSize = savedData.skewSize;



                    if (typeof savedData.rotation !== 'undefined') this._rotate(savedData.rotation);
                    //console.log("Draw - restore setting");
                    this.draw();
                ...
                value: function drawGridHelper(imageFile, size) {
                    ...
                    if (this._gridHelper) {
                        ...
                        this.removeAllTiles();
                        //console.log("Calling render from 12070 | drawgridhelper");
                        this._render();
                        ...
                        image.onload = function () {
                            ...
                            //console.log("Calling render from 12110 | drawgridhelper image");
                            _this54._render();
    ...
    function Backlight(canvas, surfaces, engine2d) {
        ...
        function show(surface) {
            ...
            if (!backLightedSurface || backLightedSurface !== surface) {
                ...
                //CODE UPDATED BY TPS
                UI.updateCanvasStyle(surface._surfaceData.custom_surface_name);
    ...
    var SurfaceSelectButton = function () {
        ...
        function SurfaceSelectButton(maskPoints, width, height, onClick) {
            ...
            if (UI.isMobileDevice() && window.JsConstants.config.surfaceSelectBrushButtons) {
                ...
                element.style.left = center.x / (width || 2000) * 100 + '%';

                element.style.top = center.y / (height || 1250) * 100 + '%';
    ...
    var Room2d = function (_Room) {
        ...
        function Room2d(data) {
            ...
            var _this55 = _possibleConstructorReturn(this, (Room2d.__proto__ || Object.getPrototypeOf(Room2d)).call(this, data));



            _this55._engineType = '2d';



            _this55._createSurfaces();



            _this55._restoreSettings(data.roomSettings);
        ...
        _createClass(Room2d, [{
            ...
            value: function _createView() {
                ...
                this.canvas.addEventListener('click', function (event) {
                    ...
                    return _this56._sceneMouseClick(event);
                ...
                this._engine2d.loadImages(this, RoomLoadProgress.end, RoomLoadProgress.progress, this.tiledSurfaces);
            ...
            value: function _render() {
                //console.log("IN _render C | 12438");
                //console.log("CALL RENDER 12442 FROM CREATE VIEW");
                //console.log(this);
            ...
            value: function _restoreSettings(settings) {
                    ...
                    TilesList.some(function (tile) {
                        ...
                        if (tileName === name) {
                            ...
                            if (!settings) {
                                ...
                                settings = { surfaces: Array(_this57.tiledSurfaces.length).fill(undefined) };
                ...
                if (settings && Array.isArray(settings.surfaces)) {
                    ...
                    settings.surfaces.forEach(function (surfaceSettings, index) {
                        ...
                        if (tile0 && _this57.tiledSurfaces[index].type === tile0.surface) {
                        ...
                        _this57.tiledSurfaces[index].restoreSettings(surfaceSettings);
            ...
            value: function _sceneMouseClick(event) {
                ...
                if (this.tiledSurfaces.length > 0) {
                    ...
                    this.tiledSurfaces.some(function (surface) {
                        ...
                        if (_this58._engine2d.isPointOnShape(point, surface[11])) {
                            //ADDED by TPS
                            surfaceClickedByUser(surface._surfaceData.custom_surface_name);
                            //END
                            _this58._onSurfaceClick(surface);
                            ...
                            if (surface.freeDesign) {
                                ...
                                if (surface.freeDesignTile) {
                                    ...
                                    _this58._ui.alertWarning('Tile not selected.');
            ...
            value: function _createSurfaces() {
                ...
                if (Array.isArray(this.surfacesData)) {
                    this.surafaceDataButtonsArray = [];

                    this.surfacesData.forEach(function (surfaceData) {
                        //CODE BY TPS
                        surfaceData.custom_surface_name = getCustomNameOfSurfaceData(surfaceData["176"]);
                        //END
                        var surface = new TiledSurface2d(surfaceData, function () {
                            //console.log("Calling render from 12671 | createsurface");
                            return _this59._render();

                        });

                        _this59.tiledSurfaces.push(surface);


                        new SurfaceSelectButton(surface[11], surface.projection.width, surface.projection.height, function () {

                            return _this59._onSurfaceClick(surface);
	...
	var isWebSocketClosedManually = false;

	function connectWebSocket() {
		if (isWebSocketClosedManually) {
			console.warn("⏹️ WebSocket closed manually. Not reconnecting.");
			return;
		}

		ws = new WebSocket("wss://67.211.222.179:8061");
		...
		ws.onmessage = function (event) {
			...
			if (response.status === "success" && response.imageData) {
				console.log("🖼️ Image received, redirecting...");
				localStorage.setItem("uploadedRoomImage", response.imageData);
				window.location.href = "/get/ai-studio";
			}
		};
		...
		ws.onclose = function () {
			if (!isWebSocketClosedManually) {
				console.warn("⚠️ WebSocket closed by server. Not reconnecting.");
			}
		};
	}
    ...
    function sendPing() {
        setInterval(() => {
			if (ws && ws.readyState === WebSocket.OPEN) {
				...
				console.warn("⚠️ WebSocket not open. Skipping ping.");
			}
		}, 5000);
    }

    
    // Handle Image Upload (On Index Page)
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (uploadButton) {
            ...
            fileInput.addEventListener("change", function (event) {
                ...
                if (file) {
                    waitForWebSocket(() => uploadImageViaWebSocket(file));
                }
            });
        }

        // Load Image on Room Page
        if (window.location.pathname === "/get/ai-studio") {
            loadUploadedRoomImage();
        }
    });

    // Ensure WebSocket is Open Before Sending Image
    function waitForWebSocket(callback, attempts = 0) {
		if (ws.readyState === WebSocket.OPEN) {
			callback();
		} else if (attempts >= 10) {  // Stop retrying after 10 attempts
			console.error("WebSocket failed to open after multiple retries.");
		} else {
			console.warn(`Waiting for WebSocket to be ready... (Attempt ${attempts + 1}/10)`);
			setTimeout(() => waitForWebSocket(callback, attempts + 1), 500);
		}
	}

    // Upload Image via WebSocket & Redirect After Success
    function uploadImageViaWebSocket(file) {
		...
		reader.onload = function (event) {
			...
			function sendImage() {
				if (ws && ws.readyState === WebSocket.OPEN) {
					ws.send(JSON.stringify({ action: "upload", image: imageData }));
					console.log("📤 Image sent via WebSocket.");
				} else {
					console.warn("⚠️ WebSocket not open. Waiting 1 second before retrying...");
    ...
    // Load Image into Canvas in Room Page
    function loadUploadedRoomImage() {
        const imageData = localStorage.getItem("uploadedRoomImage");
        if (imageData) {
            const img = new Image();
            img.onload = function () {
            ...
            console.warn("⚠️ No uploaded room image found.");
        }
    }

    window.addEventListener('load', function () {

        new Loader().loadRoom('/get/ai-studio/', function (data) {
            currentRoom = new Room2d(data);
            return currentRoom;
