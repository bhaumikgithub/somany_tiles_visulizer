            ...
            <div class="modal-body">
                <div class="modal-body">
                    <ul class="nav nav-tabs">
                        <?php
                        $saved_rooms_present = isset($saved_rooms) && $saved_rooms->count() > 0;
                    ...
                    if ($saved_rooms_present) {
                        ...
                        foreach ($saved_rooms as $saved_room) {
                            if ($saved_room->room) {
                                $target = '';
                                $backed = '';
                                if ($saved_room->engine == 'panorama' && $saved_room->note == 'backed') {
                                    $target = ' target="_blank" ';
                                    $backed = ' (backed)';
                                }

                                echo '<a href="/room/url/' . $saved_room->urlWithParams . '" title="' . $saved_room->room->name . '" class="room-select-link" ' . $target . '>';
                                $engine_icon_img = '';
                                if (isset($saved_room->engine) && !config('app.hide_engine_icon')) {
                                    ...
                                    if (isset($engine_icon)) {
                                        $engine_icon_img = '<img src="' . $engine_icon . '" loading="lazy" alt="" width="32" class="room-image-engine-icon">';
                                    }
                                }
                                echo '<div class="room-image-holder"><img src="' . $saved_room->image . '" loading="lazy" alt="">',
                                    $engine_icon_img . '</div>',
                                    '<p>' . $saved_room->room->name . $backed . '</p>',
                                    '</a>';
                            }
                        }
                        ...
                        foreach ($rooms as $key => $sub_rooms) {
                            ...
                            foreach ($sub_rooms as $sub_room) {
                                ...
                                if (!config('app.hide_engine_icon')) {
                                    $engine_image_element = '<img src="' . $engine_icon . '" loading="lazy" alt="" width="32" class="room-image-engine-icon">';
                                }
                                echo '<a href="' . $room_link . '" title="' . $sub_room->name . '" class="room-select-link">',
                                    '<div class="room-image-holder"><img src="' . $sub_room->icon . '" loading="lazy" alt="">' . $engine_image_element . '</div>',
                                    '<p>' . $sub_room->name . '</p>',
                                    '</a>';
                            }
