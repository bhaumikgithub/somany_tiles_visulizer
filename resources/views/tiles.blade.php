...
<?php
  $config_app_product_finish = config('app.product_finish');
?>
...
function prepareExpPropsJsonString() {
    ...
    var specular = window.$('#form-tile-specular-checkbox').prop('checked');
    if (specular) expProps.specular = specular;
...
function clickSpecularCheckbox() {
    'use strict';
    if (window.$('#form-tile-specular-checkbox').prop('checked')) {
      window.$('#form-tile-specular-file-box').show();
    } else {
    ...
    try {
        ...
        for (option in expProps) {
            if (expProps.hasOwnProperty(option) && expProps[option]) {
                ...
                case 'specular':
                    window.$('#form-tile-specular-checkbox').attr('checked', expProps.specular);
                    clickSpecularCheckbox();
                    break;
...
function tileFormReset() {
    ...
    window.$('#form-tile-specular-checkbox').attr('checked', false);
...
function editTile(id) {
    ...
    window.$.ajax({
        ...
        success: function (tile) {
            ...
            window.$('#form-tile-specular-checkbox-box').show();
            clickSpecularCheckbox()
            ...
            window.$('#form-tile-specular-file-img').attr('src', getSpecularFileName(tile.file));
...
<div id="addTilesFormBlock" class="panel-body" style="display: none;">
  <form id="addTilesForm" name="addTilesForm" action="/tiles/upload" method="POST" enctype="multipart/form-data" class="form-horizontal">
    ...
    <div class="form-group" id="form-tile-specular-file-box" style="display: none;">
      <label for="form-tile-specular-file" class="col-sm-3 control-label">Specular map</label>
      <div class="col-sm-2">
        <img id="form-tile-specular-file-img" src="" alt="" class="img-thumbnail" style="max-width: 128px; max-height: 128px;cursor: pointer;" onclick="showBigTileImageModal(window.$('#form-tile-name').val() + '(specular map)', this.src)">
      </div>
      <div class="col-sm-4">
        <input type="file" name="specularFile" id="form-tile-specular-file" class="form-control" accept="image/*">
      </div>
      @if (!config('app.unlimited_image_size'))
        <span class="col-sm-3 help-block">Image must be less than 2 MB and resolution less than 2048x2048 pixels.</span>
      @endif
    ...
    <div class="form-group">
      ...
      <div id="form-tile-specular-checkbox-box" class="col-sm-2" style="display: none;">
        <label class="control-label">
          <input type="checkbox" id="form-tile-specular-checkbox" onclick="clickSpecularCheckbox()">
          Use Specular map
        </label>
      </div>
    </div>
