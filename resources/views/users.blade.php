    ...
    @if (count($users) > 0)
        ...
        <script type="text/javascript" charset="utf-8" async defer>
            ...
            function editUser(id) {
                ...
                window.$('#form-update-user-contact_no').val('');
                window.$('#form-update-user-show-room').val([]); // Clear previous selections
                ...
                window.$.ajax({
                    ...
                    success: function (user) {
                        ...
                        if (Number(user.enabled)) {
                            window.$('#form-update-user-enabled').attr('checked', true);
                        }
        ...
        <div id="updateUserForm" class="panel-body" style="display: none;">
            ...
            <form action="/user/update" method="POST" enctype="multipart/form-data" class="form-horizontal">
                ...
                <div class="form-group required">
                    ...
                    <div class="col-sm-3">
                        <input type="text" name="id" id="form-update-user-id" class="form-control" readonly="readonly"
                               required>
                    </div>
                    <div class="col-sm-3">
                        <label><input type="checkbox" name="enabled" id="form-update-user-enabled" value="1">
                            Enabled</label>
                    </div>
                </div>
                ...
                <div class="form-group required">
                    ...
                    <div class="col-sm-6">
                        <input type="text" name="name" id="form-update-user-name" class="form-control"
                ...
                <div class="form-group required">
                    ...
                    <div class="col-sm-6">
                        <input type="text" name="email" id="form-update-user-email" class="form-control"
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <img id="form-update-user-avatar-img" src="" alt="" class="img-thumbnail"
                             style="max-width: 128px; max-height: 128px;cursor: pointer;"
                             onclick="showBigAvatarModal(window.$('#form-update-user-name').val(), this.src)">
                    </div>
                    <div class="col-sm-4">
                        <input type="file" name="avatar" id="form-update-user-avatar" accept="image/*"
                               class="form-control">
                    </div>
                ...
                <div class="form-group required">
                    <label for="form-update-user-contact_no" class="col-sm-3 control-label">Mobile</label>
                    <div class="col-sm-6">
                        <input type="text" name="contact_no" id="form-update-user-contact_no" class="form-control"
                               placeholder="Enter Mobile Number" pattern="\d{10}" maxlength="10" required>
                        <span class="error" style="color: red; display: none;">Please enter a valid 10-digit mobile number.</span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="col-sm-offset-3 col-sm-6">
                        <button type="button" class="btn btn-default"
                                onclick="deleteUser(window.$('#form-update-user-id').val());" title="Remove User"><span
                                    class="glyphicon glyphicon-trash" aria-hidden="true"></span></button>
                        <button type="button" class="btn btn-default"
                                onclick="showResetPasswordModal(window.$('#form-update-user-id').val(), window.$('#form-update-user-name').val())">
                            Reset password
                        </button>
                        <div class="pull-right">
                            <button type="submit" class="btn btn-primary">Update user</button>
                            <button type="button" class="btn btn-default" onclick="$('#updateUserForm').hide('fast');">
                                Cancel
                            </button>
                        </div>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button id="confirmDialogSubmit" type="submit" class="btn btn-primary"
                                onclick="$('#usersForm').submit();">Confirm
                        </button>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="/user/reset/password" method="POST" class="form-horizontal">
                        ...
                        <div class="modal-body" style="text-align: center;">
                            <div class="form-group required">
                                <label for="form-reset-user-password" class="col-sm-3 control-label">New
                                    password</label>
                                <div class="col-sm-6">
                                    <input type="password" name="password" id="form-reset-user-password"
                                           class="form-control" placeholder="New password" maxlength="255" required>
                                </div>
                            </div>
                            <div class="form-group required">
                                <label for="form-reset-user-password-confirm" class="col-sm-3 control-label">Confirm
                                    password</label>
                                <div class="col-sm-6">
                                    <input type="password" name="password_confirmation"
                                           id="form-reset-user-password-confirm" class="form-control"
                                           placeholder="Confirm password" maxlength="255" required>
                                </div>
            ...
            <div class="panel-body">
                <table class="table table-striped">
                    ...
                    @foreach ($users as $user)
                        <tr>
                            <td class="table-text">
                                <input type="checkbox" name="" value="{{ $user->id }}"
                                       onchange="addCheckedUser(this.value, this.checked);">
                            </td>
                            <td class="table-text">
                                <img src="{{ $user->avatar }}" alt="" class="img-thumbnail" role="button"
                                     style="max-width: 64px; max-height: 64px;"
                                     onclick="showBigAvatarModal('{{ $user->name }}', this.src)">
                            </td>
                            <td class="table-text bold"><a href="#" title="Edit" onclick="editUser( {{ $user->id }} )"
                                                           title="Edit">{{ $user->name }}</a></td>
                            ...
                            <td class="table-text">@if ($user->enabled)
                                    Yes
                                @else
                                    No
                                @endif</td>
